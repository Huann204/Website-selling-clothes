...
const Header = ({ onOpenMenu }) => {
  ...
  return (
    <header className="h-[70px] px-[10px] md:px-[50px] md:h-[100px] bg-white fixed top-0 left-0 z-[99] transition-all duration-300 w-full shadow-[0px_8px_24px_rgba(150,158,166,0.2)]  ">
      <div className=" h-full md:max-w-[1620px] mx-auto flex items-center justify-between w-full">
        <div className="md:hidden" onClick={() => onOpenMenu()}>
          <BiMenuAltLeft size={22} />
        </div>
        <Link
          to={"/"}
          className="font-semibold text-[22px] flex flex-1 justify-center md:hidden cursor-pointer "
        >
          SSSTUTER
        </Link>
        ...
        <div className="flex items-center gap-[10px] md:gap-9 ">
          <button
            type="button"
            aria-label="Tìm kiếm"
          ...
          <button type="button" aria-label="Yêu thích">
            <BiHeart className="size-5 md:size-6" />
          </button>
          <NavLink to={"/cart"} className="relative" aria-label="Giỏ hàng">
            <BiShoppingBag className="size-5 md:size-6" />
