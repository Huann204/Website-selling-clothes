...
export default function AdminProductEdit() {
  ...
  const updateVariant = useCallback((index, data) => {
    setVariants((prev) => {
      const newArr = [...prev];
      newArr[index] = data;
      return newArr;
          ...
          <div className="w-full px-4 sm:px-6 lg:px-8 2xl:px-10 py-3 sm:py-4 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                disabled={loading}
                className="inline-flex items-center justify-center gap-2 rounded-xl bg-slate-900 px-3 py-2 text-sm text-white hover:bg-slate-800 disabled:opacity-50"
              >
...
function VariantCard({ variant, onChange, onRemove }) {
  const [color, setColor] = useState(variant?.color?.name || "");
  const [stocks, setStocks] = useState(() => {
    const initialStocks = { S: 0, M: 0, L: 0, XL: 0 };
    if (variant?.sizes) {
      variant.sizes.forEach(({ size, stock }) => {
        initialStocks[size] = stock || 0;
      });
    }
    return initialStocks;
  });

  // cập nhật lên cha mỗi khi thay đổi (loại bỏ onChange khỏi dependencies)
  useEffect(() => {
    onChange({
      color: { name: color },
  ...
  }, [color, stocks]); // Chỉ listen color và stocks
